    ...
    if(!prodectExist){
      ...
      if(req.files && req.files.length>0){
        for(let i=0;i<req.files.length;i++){
          ...
          const resizedImagePath = path.join('public','uploads','product-imgs',req.files[i].filename)
...
const deletSingleImage = async (req,res)=>{
  try {
    ...
    const imagePath = path.join('public','uploads','product-imgs',imageNameToServer)
